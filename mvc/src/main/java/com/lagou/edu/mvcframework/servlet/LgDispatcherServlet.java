...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private static final String accessParam = "username";
    ...
    // security的map
    private Map<String, List<String>> handlerUserListMap = new HashMap<>();
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            // 扫描Security注解
            List<String> typeUserList = new ArrayList<>();
            if (aClass.isAnnotationPresent(Security.class)) {
                String[] userArray = aClass.getAnnotation(Security.class).value();
                typeUserList = Arrays.asList(userArray);
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (!method.isAnnotationPresent(Security.class)) {
                    handlerUserListMap.put(url, typeUserList);
                } else {
                    String[] methodUserArray = method.getAnnotation(Security.class).value();
                    List<String> methodUserList = Arrays.asList(methodUserArray);

                    ArrayList<String> retainUserList = new ArrayList<>(typeUserList);
                    retainUserList.retainAll(methodUserList);

                    handlerUserListMap.put(url, retainUserList);
                }
...
//        method.invoke() //
        ...
        // security注解拦截
        String requestURI = req.getRequestURI();
        if (handlerUserListMap.containsKey(requestURI)) {
            List<String> securityUsernameList = handlerUserListMap.get(requestURI);

            if (!parameterMap.containsKey(accessParam)) {
                resp.getWriter().write("this username has no permission to access");
                return;
            }
            String[] usernames = parameterMap.get(accessParam);
            List<String> usernameList = Arrays.asList(usernames);

            // 交集
            List<String> retainList = new ArrayList<>(securityUsernameList);
            retainList.retainAll(usernameList);
        ...
        for(Map.Entry<String,String[]> param: parameterMap.entrySet()) {
            ...
            String value = StringUtils.join(paramValues, ",");  // 如同 1,2
